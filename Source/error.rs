...
pub enum Error {
	...
	#[error(
		"Unsupported application architecture, expected one of `x86`, \
	...
	#[error(
		"the platform `{0}` was not found on the response `platforms` object"
	)]
	...
	#[error(
		"The signature {0} could not be decoded, please check if it is a \
		 valid base64 string. The signature must be the contents of the \
		 `.sig` file generated by the Tauri bundler, as a string."
	)]
	...
	/// Temp dir is not on same mount mount. This prevents our updater to rename
	/// the AppImage to a temp file.
	...
	#[error(
		"The configured updater endpoint must use a secure protocol like \
		 `https`."
	)]
