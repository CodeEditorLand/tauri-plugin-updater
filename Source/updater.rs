...
pub struct Updater {
	...
	target:String,
	// The value we search if the updater server returns a JSON with the
	// `platforms` object
	json_target:String,
	headers:HeaderMap,
...
impl Updater {
	pub async fn check(&self) -> Result<Option<Update>> {
		...
		for url in &self.endpoints {
			// replace {{current_version}}, {{target}} and {{arch}} in the
			// provided URL this is useful if we need to query example
			// https://releases.myapp.com/update/{{target}}/{{arch}}/{{current_version}}
			// will be translated into ->
			// https://releases.myapp.com/update/darwin/aarch64/1.0.0
			// The main objective is if the update URL is defined via the
			// Cargo.toml the URL will be generated dynamically
...
impl Update {
	...
	///
	/// We should have an AppImage already installed to be able to copy and
	/// install the extract_path is the current AppImage path
			...
			if let Some(tmp_dir_location) = tmp_dir_location() {
				...
				if extract_path_metadata.dev() == tmp_dir_metadata.dev() {
					...
					if infer::archive::is_gz(bytes) {
						...
						// we extract our signed archive into our final
						// directory without any temp file
						let archive = Cursor::new(bytes);
						...
						// if we have not returned early we should restore the
						// backup
...
mod tests {
	...
	fn it_escapes_correctly() {
		...
		// `LAUNCHAPPARGS=\"{HERE}\"`. This is the first quote level.
		// To escape a quotation mark we use a second quotation mark, so "" is
