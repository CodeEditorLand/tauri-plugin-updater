...
use tauri::{Resource, utils::platform::current_exe};
...
///
/// This type can have one of two shapes: Server Format (Dynamic Format) and
/// Static Format.
#[derive(Debug, Clone)]
...
impl Update {
	...
	#[cfg(mobile)]
	fn install_inner(&self, _bytes:&[u8]) -> Result<()> { Ok(()) }
}
...
impl Update {
	...
	/// tauri-bundler │   └──[AppName]_[version]_x64.msi           #
	/// Application MSI ├── [AppName]_[version]_x64-setup.exe.zip          #
	/// ZIP generated by tauri-bundler │   └──[AppName]_[version]_x64-setup.exe
	/// # NSIS installer └── ...
...
impl Update {
	...
	/// tauri-bundler │   └──[AppName]_[version]_amd64.AppImage        #
	/// Application AppImage ├── [AppName]_[version]_amd64.deb                #
	/// Debian package └── ...
	fn install_inner(&self, bytes:&[u8]) -> Result<()> {
			...
			if let Some(tmp_dir_location) = tmp_dir_location() {
				...
				if extract_path_metadata.dev() == tmp_dir_metadata.dev() {
					...
					if infer::archive::is_gz(bytes) {
						...
						// we extract our signed archive into our final directory without any temp
						// file
						let archive = Cursor::new(bytes);
						...
						for mut entry in archive.entries()?.flatten() {
							if let Ok(path) = entry.path() {
								if path.extension() == Some(OsStr::new("AppImage")) {
									// if something went wrong during the extraction, we should
					...
					{
						Err(err) => {
							// if something went wrong during the extraction, we should restore
							// previous app
							std::fs::rename(tmp_app_image, &self.extract_path)?;
	...
	fn is_deb_package(&self) -> bool {
		...
		let in_system_path =
			self.extract_path.to_str().map(|p| p.starts_with("/usr")).unwrap_or(false);
	...
	fn install_deb(&self, bytes:&[u8]) -> Result<()> {
		...
		for tmp_dir_location in tmp_dir_locations {
			if let Some(path) = tmp_dir_location() {
				if let Ok(tmp_dir) =
					tempfile::Builder::new().prefix("tauri_deb_update").tempdir_in(path)
				{
	...
	fn try_install_with_privileges(&self, deb_path:&Path) -> Result<()> {
		...
		if status.success() { Ok(()) } else { Err(Error::DebInstallFailed) }
	}
...
impl Update {
	...
	/// tauri-bundler │   └──[AppName].app                         # Main
...
mod tests {
	...
	fn it_escapes_correctly() {
		...
		// The output of escape_msi_property_args() will be used in
		// `LAUNCHAPPARGS=\"{HERE}\"`. This is the first quote level. To escape a
		// quotation mark we use a second quotation mark, so "" is interpreted as "
		// later. This means that the escaped strings can't ever have a single
		// quotation mark! Now there are 3 major things to look out for to not break
		// the msiexec call:
