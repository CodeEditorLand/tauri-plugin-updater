...
//!
//! - Supported platforms: Windows, Linux and macOS.crypted database and secure
//!   runtime.
...
impl<R:Runtime, T:Manager<R>> UpdaterExt<R> for T {
	fn updater_builder(&self) -> UpdaterBuilder {
		...
		let UpdaterState { config, target, version_comparator } =
	...
	fn updater(&self) -> Result<Updater> { self.updater_builder().build() }
}
